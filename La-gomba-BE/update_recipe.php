        ...
        <form action="actions/a_update_recipe.php" method ="post" id="recipeDataUpdate">
            ...
            <?php
                ...
                <div id="recipes_steps_'.$i.'" name="recipes_steps">
                    <label>Step'.$i.': </label>
                    <input type="text" class="form-control mb-2" id="recipes_description" name="recipes_description[]" value="'.$rowsteps['description'].'">
                    <input onclick="remove(recipes_steps_'.$i.')" type="button" class="btn btn-danger mb-2" value="remove">
                </div>';
                $i++;
                }
            ?>
            </div>
            <div class="form-group col-sm-8 mx-auto mt-4">
                <button id="add_steps" class="btn btn-warning">Add New Step</button>
            </div>
            <hr class="border border-warning mt-4">
            ...
            <div class="form-group col-md-8 mb-2 mx-auto" id="add_ings_div">
             <?php
                ...
                <div id="recipes_ingredients_'.$i.'" name="recipes_ingredients" >
                    <label class="border border-warning mt-4 p-2">Ingredient '.$i.':</label><br>
                    <label>Amount '.$i.':</label>
                    <input type="text" class="form-control mb-2" id="ingredients" name="ing_amount[]"  value="'.$rowingr['amount'].'">
                    <label>Unit '.$i.':</label>
                    <input type="text" class="form-control mb-2" name="ing_unit[]" value="'.$rowingr['unit'].'">
                    <label>Name '.$i.':</label>
                    <input type="text" class="form-control mb-2" name="ing_description[]" value="'.$rowingr['description'].'">
                    <input onclick="removeing(recipes_ingredients_'.$i.')" type="button" class="btn btn-danger" value="remove">
                </div>
                ';
                $i++;
                }
            ...
            <div class="form-group col-sm-8 mx-auto mt-4">
                <button id="add_ingredients" class="btn btn-warning">Add New Ingredient</button>
            </div>
            <hr class="border border-warning mt-4">
            ...
            <div class="form-row justify-content-center mb-5">
                ...
                <form method="post" action="" enctype="multipart/form-data" id="myform">
                    <div class='preview-update col-sm-4 mb-2 w-50 recimg'>
                        <img src="<?php echo $data['image'] ?>" id="img" class="w-100" >
                    </div>
            ...
            <div class="form-group col-md-5 mx-auto">
                <input type="submit" class="btn btn-success form-control font-weight-bold" value="Update Recipe" id="recipeDataUpdateButton">
            </div>
        </form>
    </div>
   ...
   function removeing(data) {
    ...
    $("#add_steps").click(function (event) {
        ...
        $("#add_steps_div").append('<div id="recipes_steps_'+$i+'" name="recipes_steps">'+
                    '<label>Step'+$i+': </label>'+
                    ' <input type="text" class="form-control mb-2" id="recipes_description" '+
    ...
    $("#add_ingredients").click(function (event) {
        ...
        $("#add_ings_div").append('<div id="recipes_ingredients_'+$i+'" name="recipes_ingredients">'+
                    '<label class="border border-warning mt-4 p-2">Ingredient '+$i+':</label><br><label>Amount '+$i+':</label>'+
                    '<input type="text" class="form-control mb-2" id="ingredients" '+
                    ...
                    'type="button" class="btn btn-danger mb-2" value="remove">'+
               ' </div>');
    });
</script>